...
// fork form github.com/go-redis/redis/extra/rediscmd/v8 for increasing the intercept length of cmd key
...
func appendArg(b []byte, v interface{}) []byte {
	...
	case string:
		...
		return appendUTF8String(b, []byte(v))
	case []byte:
		if len(v) > argLenLimit {
			v = v[:argLenLimit]
		}
