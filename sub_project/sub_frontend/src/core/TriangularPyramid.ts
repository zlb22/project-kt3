...
class TriangularPyramidProp {
  ...
  createContext(data: any, canvas: any) {
    const { left, top, src, type, name } = data
    const triangularPyramid = (fabric.Image as any).fromURL(src, function (img: any) {
      // 图片原始宽度和高度
...
export default TriangularPyramidProp
