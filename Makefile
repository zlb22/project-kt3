...
# Commands:
#   HTTPS (existing): start, stop, status
#   DEV (new): dev, dev-stop, dev-status, switch-dev
#   PROD (new): prod-build, switch-prod
#
# You can override the following variables when invoking make, e.g.:
#   make switch-dev A_LAN_IP=192.168.1.10
#   make switch-prod API_DOMAIN=api.example.com APP_DOMAIN=app.example.com KETI3_DOMAIN=topic-three.example.com MINIO_DOMAIN=minio.example.com

A_LAN_IP        ?= 127.0.0.1
API_DOMAIN      ?= api.example.com
APP_DOMAIN      ?= app.example.com
KETI3_DOMAIN    ?= topic-three.example.com
MINIO_DOMAIN    ?= minio.example.com

.PHONY: start stop status dev dev-stop dev-status switch-dev switch-prod prod-build

# ---------------- Existing HTTPS dev (self-signed) ----------------
